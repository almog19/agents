#pip install langchain langchain_ollamapip  langgraph ollama grandalf
...
def final_node(state: AgentState):
    ...
    instruction = SystemMessage(content="""
        Based on the tool outputs above, generate a final report in RAW JSON format, if information is missing specify it:
        {
          ...
          "price": "float",
          "currency": "string",
          "tool_calls_order": ["list of tools called in order"]
        }
...
You MUST use tools.
If tools fail → return missing data, and specify the tool has failed and why.
Never hallucinate.
Output JSON only.
"""),
